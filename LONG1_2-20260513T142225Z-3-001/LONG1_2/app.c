/***************************************************************************//**
 * @file
 * @brief App: AHT20 + LCD + BLE advertise + UART log (không dùng %f)
 ******************************************************************************/
...
static uint32_t    sensor_period_ms = 1000;   // 2s
...
static void update_ble_payload(float temp, float hum)
{
  ...
  adv_data[len++] = 1 + 2 + 4; // length = type + 2 byte company ID + 4 dữ liệu
...
static void sensor_timer_cb(app_timer_t *timer, void *data)
{
  ...
  if (aht20_read(I2C_HANDLE, &current_temp, &current_hum) == SL_STATUS_OK) {
    ...
    // In UART không dùng %f
    // Dùng 1 số lẻ thập phân: nhân 10
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (evt_id) {
    case sl_bt_evt_system_boot_id:
      ...
      // Chu kỳ quảng bá ~50ms (80 * 0.625 ms)
      sc = sl_bt_advertiser_set_timing(adv_handle,
                                       80, 80,
                                       0, 0);
      app_assert_status(sc);

      // Phát trên cả 3 kênh 37,38,39
      sc = sl_bt_advertiser_set_channel_map(adv_handle, 7);
      app_assert_status(sc);

      // Payload ban đầu (giá trị ảo, sẽ được cập nhật sau)
      update_ble_payload(25.3f, 64.7f);

      // Bắt đầu quảng bá non-connectable
...
{
  // Không cần xử lý gì thêm trong vòng lặp chính
}
